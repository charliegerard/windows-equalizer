...
let analyser, bufferLength, dataArray, screenX, base;
let allWindows = [];
...
window.onload = () => {
    ...
    if(isMobile){
        const mobileMessage = document.getElementsByClassName('mobile')[0];
    ...
    window.addEventListener('keydown', e => e.keyCode === 81 && quit());
} 
...
const quit = () => {
    ...
    window.cancelAnimationFrame(updateWindows);
}

const updateWindows = () => {
    window.requestAnimationFrame(updateWindows);
...
const start = () => {
    ...
    const sound = document.getElementsByTagName('audio')[0].src;
